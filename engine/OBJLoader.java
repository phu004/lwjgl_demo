...
public class OBJLoader {
		...
		public static RawModel loadObjModel(String fileName) {
			...
			try {
				...
				System.out.println("Could't load file");
				e.printStackTrace();
			}
		...
		private static void processVertex(String[] vertexData, List<Integer> indices, List<Vector2f> textures, List<Vector3f> normals, float[] textureArray, float[] normalArray) {
			...
			int currentTexPointer = 1;
			if(!vertexData[1].equals(""))
				currentTexPointer =Integer.parseInt(vertexData[1]);
				
			Vector2f currentTex = textures.get(currentTexPointer - 1);
